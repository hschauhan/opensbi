...
int ras_mpxy_init(void *fdt, int nodeoff)
{
	int rc, len;
	...
	mpxy_node_off = fdt_node_offset_by_compatible(fdt, -1, "riscv,sbi-ras-agent-mpxy-channel");
	if (mpxy_node_off < 0)
		return SBI_ENOENT;

	chan_id_p = fdt_getprop(fdt, mpxy_node_off, "channel-id", &len);
	if (!chan_id_p)
		return SBI_ENOENT;
