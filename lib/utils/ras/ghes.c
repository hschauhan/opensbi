...
static u64 *acpi_ghes_gas_register_alloc(void)
{
	...
	gas_register_allocs[gai] = (u64 *)addr;
	gai++;
...
int acpi_ghes_new_error_source(uint64_t err_src_id, uint64_t sse_v)
{
	...
	rar_addr = (u64)((u64)sblock+sizeof(acpi_ghes_status_block));
	...
	*baddr = (u64)sblock;
	...
	err_src->ghes.gas.address = (u64)baddr; /* GAS register address */
...
void acpi_ghes_record_errors(uint8_t source_id, acpi_ghes_error_info *einfo)
{
	 ...
	 */
	gas = (u64 *)err_src->ghes.gas.address;
	sblock = (acpi_ghes_status_block *)(*gas);
