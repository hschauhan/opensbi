...
int acpi_ghes_new_error_source(uint64_t err_src_id)
{
	...
	err_src->ghes.gas.address = (uint64_t)sblock;
	err_src_alloc_idx++;
			       ...
			       uint64_t ack_preserve, uint64_t ack_write)
{
	...
	resp = *((volatile uint64_t *)read_ack_register->address);
	resp &= ack_preserve;
	resp &= ack_write;
...
void acpi_ghes_record_errors(uint8_t source_id, acpi_ghes_error_info *einfo)
{
	 ...
	 */
	sblock = (acpi_ghes_status_block *)err_src->ghes.gas.address;
