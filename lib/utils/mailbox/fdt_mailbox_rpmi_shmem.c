...
static int __smq_rx(struct smq_queue_ctx *qctx,
		    u32 service_group_id, struct mbox_xfer *xfer)
{
	...
	if ((sizeof(u32) * args->rx_endian_words) >
	    (qctx->slot_size - sizeof(struct rpmi_message_header)))
		return SBI_EINVAL;
	...
	pos = readidx;
	while (pos != writeidx) {
		src = (void *)qctx->queue->buffer + (pos * qctx->slot_size);
	...
	if (pos != readidx) {
		src = (void *)qctx->queue->buffer + (pos * qctx->slot_size);
		dst = (void *)qctx->queue->buffer + (readidx * qctx->slot_size);
		for (i = 0; i < qctx->slot_size / sizeof(u32); i++) {
			tmp = ((u32 *)dst)[i];
			((u32 *)dst)[i] = ((u32 *)src)[i];
			((u32 *)src)[i] = tmp;
		}
	...
	msg = (void *)qctx->queue->buffer + (readidx * qctx->slot_size);
	if (no_rx_token)
		    ...
		    u32 service_group_id, struct mbox_xfer *xfer)
{
	...
	dst = (char *)qctx->queue->buffer + (writeidx * qctx->slot_size);
		  ...
		  u32 queue_id, u32 service_group_id, struct mbox_xfer *xfer)
{
	...
	do {
		...
		ret = __smq_rx(qctx, service_group_id, xfer);
		  ...
		  u32 queue_id, u32 service_group_id, struct mbox_xfer *xfer)
{
	...
	do {
		spin_lock(&qctx->queue_lock);
		ret = __smq_tx(qctx, mctl->mb_regs, service_group_id, xfer);
		spin_unlock(&qctx->queue_lock);
		if (!ret)
			return 0;
				     ...
				     void *fdt, int nodeoff)
{
	...
	for (qid = 0; qid < mctl->queue_count; qid++) {
		...
		prop_slotsz = fdt_getprop(fdt, nodeoff,
					  "riscv,slot-size", &len);
		if (!prop_slotsz)
			return SBI_ENOENT;

		qctx->slot_size = fdt32_to_cpu(*prop_slotsz);
		if (qctx->slot_size < RPMI_MSG_SIZE_MIN) {
			sbi_printf("%s: slot_size < supported message size\n",
				   __func__);
			qctx->slot_size = RPMI_MSG_SIZE_MIN;
		}
		/* calculate number of slots in each queue */
		qctx->num_slots =
			(reg_size - RPMI_QUEUE_HEADER_SIZE) / qctx->slot_size;
